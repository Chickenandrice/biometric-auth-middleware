...
class AuthService:
    ...
    async def authorize_with_payload(
        self,
        request: AuthorizeRequest,
        payload: VerificationPayload,
        db: AsyncSession,
    ) -> AuthorizeResponse:
        """Authorize using a verification payload from a client relay (BLE/SDK), not server transport."""
        reason_codes: list[str] = []
        risk_level = request.risk_level
    ...
    async def _log_decision(
        ...
        **extra,
        ...
        ss = log_extra.get("similarity_score")
        if ss is not None:
            log_extra["similarity_score"] = round(float(ss), 4)
